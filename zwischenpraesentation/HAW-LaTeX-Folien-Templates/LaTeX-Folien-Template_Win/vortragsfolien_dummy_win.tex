...
\usepackage[latin1]{inputenc} % inputencoding (MacOS) für Umlaute und Akzente
...
\institute[MT -- HAW Hamburg]{HAW Hamburg\\ Fakultät DMI, Dept.\ Medientechnik}
...
\title[Überschrift]{Die Hauptüberschrift}
\subtitle{Und eine Unter--Überschrift}
...
\item \pause \textbf{Ha\pause nu\pause ta} \pause(Haselnusstafel):\\ \pause
    {\em der pause-Befehl kann überall gesetzt werden!}
\pause
...
Diese Folie ist mit der columns--Umgebung mehrspaltig gesetzt. Die Spaltenbreite kann frei gewählt werden (hier: linke Spalte \texttt{0.3}$\backslash$\texttt{textwidth}, rechte Spalte \texttt{0.6}$\backslash$\texttt{textwidth}). Die Schriftgröße in den zwei Listen ist ausnahmsweise $\backslash$\texttt{small}.
...
...mit Bildern im zweispaltigen Satz. Die Einbindung der Bilder wird hier mit dem Befehl pgfimage realisiert. Die Maßangabe $\backslash$\texttt{textwidth} ist relativ: außerhalb des zweispaltigen Satzes bezieht sie sich auf die gesamte zur Verfügung stehende Breite, innerhalb auf die aktuelle Spaltenbreite.

\medskip %mittelgroßer vertikaler Abstand zwischen Text und Bildern
...
wird aus Standard-Blöcken (Umgebung block) zusammengesetzt:
\end{block}
...
...mit einem exampleblock (grün) und einem alertblock (rot):
...
\pause
Mehr als drei Farben (Standardfarbe --- hier Blau --- plus Grün plus Rot für besonders wichtige Punkte) sollten nicht benutzt werde, sonst wirds zu bunt.
...
$\backslash$\texttt{uncover}$<$nr-$>$$\{$...$\}$ bekommt als Parameter die Nummer der Teil--Folie übergeben, ab der der geklammerte Bereich sichtbar wird. $\backslash$\texttt{uncover}$<$nr$>$$\{$...$\}$ enthüllt den Text nur auf einer Teilfolie, $\backslash$\texttt{uncover}$<$nr1-nr2$>$$\{$...$\}$ von der Teil--Folie nr1 bis zur Teil--Folie nr2.
\end{frame}
...
Der only-Befehl funktioniert ähnlich wie uncover:
\begin{eqnarray}\nonumber
...
Allerdings lässt $\backslash$\texttt{only}$<$nr-$>$$\{$...$\}$ den verborgenen Text komplett verschwinden; damit wird auch kein Platz reserviert. 
\end{frame}
...
Und hier die Wirkung des (komplementären) invisible--Befehls:
\begin{eqnarray}\nonumber
\only<2->{\xi_1(t) + \xi_2(t)} &\only<2->{=}& \only<2->{\xi_0\,\left(\sin(\omega_1 t)+\sin(\omega_2
t)\right)} \\ \nonumber
...
Wenn der geplante Inhalt im Standard--Layout nicht auf eine Folie passt, dann sollte man \alert<3>{nicht am Layout herumwurschteln} und schon gar nicht die Schrift verkleinern, \alert<3>{sondern den Inhalt der Folie kürzen}. Ganze Sätze gehören nicht auf die Folie.
\end{alertblock}}

\uncover<4->{
Deshalb:
\begin{itemize}
\alert<5>{\item besser Aufzählungen als Fließtext!}
\alert<6>{\item nur Stichworte!}
\alert<7>{\item niemals ganze Sätze!}
\end{itemize}}
