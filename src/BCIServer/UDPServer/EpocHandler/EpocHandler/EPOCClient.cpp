...
int EPOCClient::getDataRawData(struct RAWDATAPACKAGE * package, int index){
    ...
    package->counter =  ed_counter[index];
    package->gyroX =  ed_gyroX[index];
    package->gyroY =  ed_gyroY[index];
    package->timeStamp =  ed_timeStamp[index];
    package->func_ID =  ed_func_id[index];
    package->func_Value =  ed_func_value[index];
    package->marker =  ed_marker[index];
    package->sync_Signal = ed_sync_signal[index];
