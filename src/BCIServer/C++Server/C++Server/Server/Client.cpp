...
int Client::getEmoState(struct EMOSTATEPACKAGE * rPackage){
    int rc = 0;
    struct REQUESTPACKAGE package;
    package.mode = EMOSTATE;
...
int Client::getRawData(struct RAWDATAPACKAGE * rPackage){
    int rc = 0;
    struct REQUESTPACKAGE package;
    package.mode = RAWDATA;

    if(DEBUG){printf("Aufruf der Methode getRawData()\n");}
    ...
    rc = recvfrom(udpSocket, (char*)rPackage, sizeof(RAWDATAPACKAGE), 0, (SOCKADDR*)&remoteAddr, &remoteAddrLen);
    if(rc == SOCKET_ERROR){
        printf("Fehler: recvfrom, Fehlercode: %d\n",WSAGetLastError());
        return 1;
    }
