...
int done;
static float gOscInfo;

Driver::Driver(int index) {
    gOscInfo = 0;
    INDEX = index;
    oscServer.start(&gOscInfo);
}
...
float Driver::getAccel() {
    std::cout << "Driver::getAccel returns: " << gOscInfo << std::endl;
    return gOscInfo;
//    float allowedspeed = getAllowedSpeed(car->_trkPos.seg);
//    float gr = car->_gearRatio[car->_gear + car->_gearOffset];
//    float rm = car->_enginerpmRedLine;
//    if (allowedspeed > car->_speed_x + FULL_ACCEL_MARGIN) {
//        //std::cout << "return 1.0" << std::endl;
//        return 1.0;
//    } else {
//        //std::cout << "return" << accel_boost * allowedspeed / car->_wheelRadius(REAR_RGT) * gr / rm << std::endl;
